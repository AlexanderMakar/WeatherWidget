...
public class WeatherService : IWeatherService
{
    ...
    private const string GeoInfoUri = "geo/1.0/reverse?lat={0}&lon={1}&appid={2}";
    ...
    public async Task<Weather> GetWeatherAsync(string latitude, string longitude)
    {
        ...
        var weather = new Weather
        {
            ...
            GeoInfoResponse = await FetchGeoInfoAsync(latitude, longitude, key)
        };
    ...
    private async Task<GeoInfoResponse> FetchGeoInfoAsync(
        string latitude,
        string longitude,
        string key)
    {
        _logger.LogInformation("Fetching geo information");

        var response = await _httpClient
            .GetAsync(string.Format(GeoInfoUri, latitude, longitude, key));
        ...
        {
            throw new FetchWeatherInfoException("Couldn't get geo information, see inner exception", e);
        }

        var content = JsonConvert.DeserializeObject<List<GeoInfoResponse>>(await response.Content.ReadAsStringAsync());
